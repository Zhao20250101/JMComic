...
> 💬 默认以「聊天记录（合并转发）」卡片形式发回下载结果：群里只显示一张折叠卡片，
> 点开才看到完成提示与产物文件，既不刷屏也更隐蔽。如需直接把文件发到聊天里，
> 把配置项 `send_as_forward` 关掉即可。部分 QQ 协议端（NapCat / Lagrange）的合并转发节点
> 可能不支持文件类消息，若卡片里看不到文件或发送报错，关闭该开关即可恢复直接发送。
...
| `send_as_forward` | 以「聊天记录（合并转发）」形式发回产物，关闭则直接发文件 | `true` |
